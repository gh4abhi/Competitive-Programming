// Problem Link - https://codeforces.com/contest/1726/problem/C
...
void DFS(vector<ll> &vis, vector<vector<ll>>& adj, ll ind)
{
    vis[ind] = 1;
    for(auto i:adj[ind])
        if(vis[i]==0)
...
void its_Function() {
 ...
 for(ll i=0;i<2*n;i++)
 {
    ...
    else
    {
        ...
        if(m.count(st.top()))
        {
            adj[ok].push_back(m[st.top()]);
            adj[m[st.top()]].push_back(ok);
        }
        else
            m[st.top()] = ok;
    }
