...
ll ini,end;
for(ll i=1;i<posA.size();i++)
{
    ll curMini = posA[i]-posA[i-1];
    if(curMini<mini)
    {
        mini = curMini;
        ini = posA[i-1];
...
m2['b'] = 0;
m2['c'] = 0;
for(ll i=ini;i<=end;i++)
{
    m2[str[i]]++;
}
if(m2['a']>m2['b'] and m2['a']>m2['c'])
{
    cout<<mini+1;
    return;
}
if(str.find("abca") != string::npos or str.find("acba") != string::npos)
{
    cout<<4;
    return;
}
if (str.find("abbacca") != string::npos or str.find("accabba") != string::npos)
{
    cout<<7;
    return;
}
