...
void its_Function() {
    string str;
    getline(cin, str);
    ll x1,y1,x2,y2,xf,yf;
    cin>>x1>>y1>>x2>>y2>>xf>>yf;
    ll sum=0;

    if((x1==x2 and x1==xf and yf>min(y1,y2) and yf<max(y1,y2)) or (y1==y2 and y1==yf and xf>min(x1,x2) and xf<max(x1,x2)))
    {
         sum+=abs(x2-x1)+abs(y1-y2) + 2;
    }
    else
    {
