// Problem Link - https://codeforces.com/contest/1706/problem/B
...
void its_Function() {
 ...
 for(ll i=0;i<n;i++)
 {
    if(m.count(vect[i])==0)
        m[vect[i]] = {i-1,0};
    // cout<<vect[i]<<" "<<m[vect[i]].first<<" "<<m[vect[i]].second<<endl;
    if((i-m[vect[i]].first)&1)
        m[vect[i]] = {i,m[vect[i]].second+1};
    ans[vect[i]] = max(ans[vect[i]],m[vect[i]].second);   
 }
 for(ll i=1;i<=n;i++)
    cout<<ans[i]<<" ";
}
