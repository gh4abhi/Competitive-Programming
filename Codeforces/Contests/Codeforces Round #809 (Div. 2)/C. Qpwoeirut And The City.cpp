...
void its_Function() {
 ...
 if(n&1)
 {
    ...
    {
        if(vect[i]<=vect[i-1] or vect[i]<=vect[i+1])
        {
            ll val = max(vect[i-1],vect[i+1])+1;
            sum += (val-vect[i]);
        }
    }
    cout<<sum;
 ...
 else
 {
    ...
    for(ll i=1;i<n-1;i+=2)
    {
        if(vect[i]>vect[i-1] and vect[i]>vect[i+1])
            dp1[i] = 0;
        else
        {
            dp1[i] = max(vect[i-1],vect[i+1])+1-vect[i];
        }
        if(i>1)
            dp1[i] += dp1[i-2];
    }
    ...
    for(ll i=n-2;i>=0;i-=2)
    {
        ...
        {
            dp2[i] = max(vect[i-1],vect[i+1])+1-vect[i];
        }
    ...
    ll mini = 1e18;
    mini = min(dp1[n-3],dp2[2]);
    for(ll i=1;i<n-1;i++)
    {
        if(i+3<n-1)
        mini = min(mini,dp1[i]+dp2[i+3]);
    }
    cout<<mini;
 }
}
