...
void its_Function() {
    ll n,m;
    cin>>n>>m;
    vector<string> palindromes, nonPalindromes;
    vector<string> vect(n);
    for(ll i=0;i<n;i++)
    {
        cin>>vect[i];
        if(checkPalindrome(vect[i]))
        {
            palindromes.pb(vect[i]);
        }
        else
        {
            nonPalindromes.pb(vect[i]);
        }
    }
    /*for(auto i:nonPalindromes)
    {
//        cout<<"i "<<i<<endl;
    }*/
//    nonPalindromes = vect;
    string ans="", revAns = "";
    map<string,ll> m1;
    for(ll i=0;i<nonPalindromes.size();i++)
    {
        string temp = nonPalindromes[i];
        if(m1.count(temp)>0)
        {
            continue;
        }
...
//        cout<<temp<<endl;
        ...
        ll ind = find(full(nonPalindromes),temp) - nonPalindromes.begin();
        if(ind<nonPalindromes.size())
        {
            ans+=nonPalindromes[i];
            m1[nonPalindromes[i]]++;
            revAns = temp + revAns;
            m1[temp]++;
    ...
    for(auto i:m2)
    {
        if(i.second==1 and i.first.length()>midPal.length())
        {
            midPal = i.first;
            flag=1;
            continue;
