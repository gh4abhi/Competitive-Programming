// Problem Link - https://codeforces.com/gym/345620/problem/B
...
void its_Function() {
  ...
  for(ll i=0;i<n;i++)
  {
      for(ll j=0;j<m;j++)
      {
          ll val1 = vect[i][m-j-1];
          ll val2 = vect[n-i-1][j];
          vector<ll> temp;
          temp.pb(vect[i][j]);
          temp.pb(val1);
          temp.pb(val2);
          sort(full(temp));
          ll mid = temp[1];
          sum+=(abs(val1-mid)) + (abs(val2-mid)) + abs(vect[i][j]-mid);
          vect[i][j] = vect[i][m-j-1] = vect[n-i-1][j] = mid;
      }
  }
    cout<<sum;
}
