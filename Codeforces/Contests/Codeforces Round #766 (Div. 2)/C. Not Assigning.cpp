...
void DFS(map<ll,bool>& visited, map<ll,vector<ll>>& adj,ll start, ll flag)
{
    visited[start] = true;
...
DFS(visited, adj, start,0);
for(auto i:ans)
    cout<<val[{i.first,i.second}]<<" ";
val.clear();
}
