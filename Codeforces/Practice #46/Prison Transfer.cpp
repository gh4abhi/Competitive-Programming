// Problrm Link - https://codeforces.com/gym/354167/problem/B
...
void its_Function() {
    ...
    if(pos.size()==1)
    {
        ...
        {
            ll a = max(0ll, pos[0] - (c-1));
            cout<<a + max(((n-1) - pos[0] - (c-1)),0ll);
            return;
        }
    }
//    cout<<endl;
    ...
    for(ll i=0; i<pos.size();i++)
    {
        if(i==0)
        {
            if(pos[i]!=0)
            {
                ways += max(((pos[i]-start) - (c-1)),0ll);
            }
            start = pos[i];
            continue;
        }
        ways += max((pos[i]-start-1) - (c-1),0ll);
        start = pos[i];
        if(i==pos.size()-1)
        {
            if(pos[i]!=n-1)
            {
                ways+= max((((n-1)-pos[i]) - (c-1)),0ll);
            }
            start = pos[i];
        }
    }
