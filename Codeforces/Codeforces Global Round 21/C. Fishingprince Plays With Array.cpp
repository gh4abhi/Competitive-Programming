// Problem Link - https://codeforces.com/contest/1696/problem/C
...
vector<pair<ll,ll>> solve(vector<ll>& a, ll m)
{
    ...
    for(auto i:a)
    {
        ...
        while(val%m==0)
            val/=m;
        count = i/val;
        if(vect.size() and vect.back().first==val)
            vect.back().second += count;
        else
            vect.push_back({val,count});
    }
    return vect;
}
