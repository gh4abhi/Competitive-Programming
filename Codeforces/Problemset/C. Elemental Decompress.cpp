// Problem Link - https://codeforces.com/problemset/problem/1768/C
...
void its_Function() {
  ...
  cin>>n;
  vector<ll> vect(n);
  ll maxi = 0, one = 0, maxE = -1;
  unordered_map<ll,ll> m;
  forl(n)
  {
    cin>>vect[i];
    m[vect[i]]++;
    maxE = max(maxE,vect[i]);
    maxi = max(maxi,m[vect[i]]);
    if(vect[i]==1)
        one = m[1];
  }
  if(one>1 or maxi>2 or maxE<n)
  {
    cout<<"NO";
    return;
  }
  vector<ll> op1(n), op2(n);
  ...
  {
    if(m1.count(vect[i]))
        op2[i] = vect[i], m2[vect[i]]++,a2.erase(vect[i]);
    else
        op1[i] = vect[i], m1[vect[i]]++,a1.erase(vect[i]);
  }
