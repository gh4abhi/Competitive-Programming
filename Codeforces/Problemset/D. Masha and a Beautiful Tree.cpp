...
ll f(ll start, ll end, vector<ll> &vect)
{
    ll mid = start + (end-start)/2;
    if(start==end)
        return 0;
    if(vect[mid]>vect[mid+1])
        return 1 + f(start,mid,vect) + f(mid+1,end,vect);
    else
        return f(start,mid,vect) + f(mid+1,end,vect);
}  
...
void its_Function() {
    ...
    {
        vector<ll> a;
        for(ll i=1;i<op.size();i+=2)
        {
            if(abs(op[i]-op[i-1])!=ok)
            {
                cout<<-1;
                return;
            }
            a.pb(min(op[i],op[i-1]));
        }
        ok*=2;
        op = a;
    }
