...
void its_Function() {
 string a,s;
 cin>>a>>s;
 ll i = s.length()-1, j = a.length()-1;
 string front = "", back = "";
 string ans = "";
 while(i>=0 and j>=0)
 {
    if(front.length()==0 and i>=0)
        front = s[i--] + front;
    if(back.length()==0 and j>=0)
        back = a[j--] + back;
    /*if(front[0]=='0')
    {
        if(i>=0)
 ...
 while(i>=0)
    ans = s[i--] + ans;
 // cout<<ans;
 ll tempAns = stoll(ans);
 ll value = stoll(a);
 string check = "";
 // cout<<tempAns<<" "<<value<<endl;
 while(tempAns and value)
 {
    ll t1 = value%10;
    ll t2 = tempAns%10;
    ll t3 = t1 + t2;
    check = to_string(t3) + check;
    value/=10;
    tempAns/=10;
 }
