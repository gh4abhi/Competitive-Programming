// Problem Link - https://codeforces.com/gym/346083/problem/B
...
void its_Function() {
    ...
    for(ll i=n-1;i>=0;i--)
    {
        for(ll j=m-1;j>=0;j--)
        {
            if(vect[i][j]==0)
            {
                vect[i][j] = min(vect[i+1][j]-1,vect[i][j+1]-1);
                sum+=vect[i][j];
            }
            if(i<n-1 and vect[i][j]>=vect[i+1][j])
            {
                cout<<-1;
                return;
            }
            if(j<m-1 and vect[i][j]>=vect[i][j+1])
            {
                cout<<-1;
                return;
            }
