// Problem Link - https://www.codechef.com/submit/ADJSUMPAR?tab=statement
...
void its_Function() {
 ll n;
 cin>>n;
 vector<ll> vect(n), vis1(n),vis2(n);
 forl(n)
    cin>>vect[i];
 if(vect[0]==0)
 {
...
/*  for(auto i:vis1)
 ...
 for(ll i=2;i<n;i++)
 {
    ...
    if(i!=n)
        ...
        b = vect[0];
    if(a==0)
        vis1[i] = vis1[i-1], vis2[i] = vis2[i-1];
    else
        vis1[i] = !vis1[i-1], vis2[i] = !vis2[i-1];
 }
/* for(auto i:vis1)
 ...
 for(auto i:vis2)
    cout<<i<<" ";
 cout<<endl;*/
 if(vect[n-1]==0 and (vis1[n-1]!=vis1[0] and vis2[n-1]!=vis2[0]))
    cout<<"NO";
 else if(vect[n-1]==1 and (vis1[n-1]==vis1[0] and vis2[n-1]==vis2[0]))
    cout<<"NO";
else
    cout<<"YES";
}
