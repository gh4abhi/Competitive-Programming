// Problem Link - https://www.codechef.com/submit/TREEXOR?tab=statement
...
ll DFS(map<ll,vector<ll>>& adj, map<ll,ll>& nodes, ll &k, ll ind)
{
    if(adj[ind].size()==0)
    {
        if(k)
    ...
    for(auto j:adj[ind])
    {
        if(flag==0)
        {
            val = DFS(adj,nodes,k,j);
            flag = 1;
        }
        else
            {
                ll op = DFS(adj,nodes,k,j);
                val = val^op;
            }
    }
    if(k)
